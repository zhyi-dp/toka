...
std::string Sema::checkExprStr(Expr *E) {
  ...
  if (auto *Num = dynamic_cast<NumberExpr *>(E)) {
    ...
    return checkBinaryExpr(Bin)->toString();
          ...
          if (varExpr) {
            ...
            if (CurrentScope->findSymbol(varExpr->Name, infoPtr)) {
              ...
              if (infoPtr->TypeObj) {
                ...
                bool isValNull =
                    infoPtr->IsValueNullable; // Should be false now
      ...
      if (!m_ControlFlowStack.empty() && !m_ControlFlowStack.back().IsLoop &&
          !m_ControlFlowStack.back().Label.empty()) {
        m_ControlFlowStack.back().IsLoop = true;
        m_ControlFlowStack.back().IsReceiver =
            isReceiver; // Sync receiver status
        tookOver = true;
      } else {
        m_ControlFlowStack.push_back({"", "void", true, isReceiver});
      }
              ...
              if (bin->Op == "=") {
                if (auto *var = dynamic_cast<VariableExpr *>(bin->LHS.get())) {
                  ...
                  isNamed = true;
                               ...
                               dynamic_cast<UnaryExpr *>(bin->LHS.get())) {
  ...
  std::shared_ptr<toka::Type> Sema::resolveType(
  ...
  bool Sema::isTypeCompatible(std::shared_ptr<toka::Type> Target,
    ...
    if (auto tShape = std::dynamic_pointer_cast<toka::ShapeType>(T)) {
      ...
      if (tName.size() >= 4 && tName.substr(0, 3) == "dyn") {
        ...
        if (!traitName.empty()) {
          ...
          if (auto sPrim =
                  std::dynamic_pointer_cast<toka::PrimitiveType>(inner))
            sName = sPrim->Name;
          else if (auto sShape =
    ...
    if (primT && primS) {
      if ((primT->Name == "i32" || primT->Name == "u32" ||
           primT->Name == "i64" || primT->Name == "u64" ||
           primT->Name == "i8" || primT->Name == "u8" || primT->Name == "i16" ||
           primT->Name == "u16") &&
          (primS->Name == "i32" || primS->Name == "u32" ||
           primS->Name == "i64" || primS->Name == "u64" ||
           primS->Name == "i8" || primS->Name == "u8" || primS->Name == "i16" ||
           primS->Name == "u16")) {
        return true;
      }

...
llvm::Function *CodeGen::genFunction(const FunctionDecl *func,
  ...
  llvm::Value *CodeGen::genVariableDecl(const VariableDecl *var) {
    ...
    if (!type) { // Only try to resolve if type hasn't been determined yet
      if (!var->TypeName.empty()) {
        type =
            resolveType(var->TypeName, var->HasPointer || var->IsUnique ||
        ...
        if (initVal->getType()->isStructTy() &&
          ...
          if (initVal->getType()->isPointerTy() &&
            ...
            if (mallocFn) {
              ...
              // 3. Create Handle
              llvm::Type *ptrTy = llvm::PointerType::getUnqual(elemTy);
              llvm::Type *refTy = llvm::PointerType::getUnqual(
                  llvm::Type::getInt32Ty(m_Context));
              llvm::StructType *st =
                  llvm::StructType::get(m_Context, {ptrTy, refTy});
              llvm::Value *u = llvm::UndefValue::get(st);
    ...
    if (sh->Kind == ShapeKind::Struct || sh->Kind == ShapeKind::Tuple) {
      ...
      for (const auto &member : sh->Members) {
        if (member.ResolvedType) {
          ...
          body.push_back(resolveType(
              member.Type, member.HasPointer || member.IsUnique ||
        ...
        for (const auto &method : trait->Methods) {
          ...
          if (f) {
            vtableMethods.push_back(
                llvm::ConstantExpr::getBitCast(f, voidPtrTy));
          } else {
  ...
  PhysEntity toka::CodeGen::genMethodCall(const toka::MethodCallExpr *expr) {
    ...
    if (!dynamicTypeName.empty()) {
      ...
      if (!traitName.empty() && m_Traits.count(traitName)) {
        ...
        if (methodIdx != -1) {
          ...
          llvm::Value *voidFuncPtr =
              m_Builder.CreateLoad(voidPtrTy, funcPtrAddr);
    ...
    if (typeName.empty()) {
      error(expr,
            "Cannot determine type for method call '" + expr->Method + "'");
      return nullptr;
    }
  ...
  void CodeGen::fillSymbolMetadata(
      TokaSymbol & sym, const std::string &typeStr, bool hasPointer,
      bool isUnique, bool isShared, bool isReference, bool isMutable,
  ...
  llvm::Type *CodeGen::resolveType(const std::string &baseType,
                                   bool hasPointer) {
    llvm::Type *type = nullptr;
    if (baseType.empty())
      return nullptr;
  ...
  void CodeGen::fillSymbolMetadata(TokaSymbol & sym,
                                   std::shared_ptr<Type> typeObj,

...
| **闭包与匿名函数 (Closure)** | | |
| `auto f = fn(x: i32) -> i32 { ... }` | 匿名函数 (全称) | 闭包与普通函数同源，默认隐式借用外部环境变量，也可显式声明参数和返回值类型 |
| `auto f = (x: i32) { ... }` | 闭包 (精简版) | 可以省略 `fn` 关键字和返回类型，在函数式迭代（如`.map`）等表达式场景中最常用 |
| `[cede a] (x) { ... }` | 显式捕获: 让渡所有权 | `[ ]` 为显式捕获指令区。`cede` 表示当前作用域彻底放弃对 `a` 的所有权，将其 Move 进闭包（解决并发生命周期逃逸标准方案） |
| `[copy ~rc] (x) { ... }` | 显式捕获: 按值拷贝 | `copy` 表示针对外部变量制作副本送入闭包（如触发 `~` 共享指针的引用计数 RC+1，或位拷贝） |
| `(x, y) { a + x }` | 捕获与传参对称性 | **铁律**：Toka 闭包的参数传递与捕获外部环境，其默认行为一律是**极其安全的隐式借用 (Borrow)** |
| 作为变量抛出或传递 | 闭包的底层特质 | 闭包的本质是含上下文状态的句柄 `fn(...)`，可毫无阻碍地传递给线程 Spawn 或作为高阶函数参数 |
| **数据结构 (Shape)** | | |
